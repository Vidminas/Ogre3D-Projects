...
Listeners::Listeners (Ogre::Root*& root, Ogre::RenderWindow*& window, Ogre::Camera*& camera, OgreBites::SdkCameraMan*& cameraMan) :
    mRoot         (root),
    mWindow       (window),
    mCamera       (camera),
...
bool Listeners::keyPressed (const OIS::KeyEvent& arg)
{
    ...
    else if (arg.key == OIS::KC_V)
    {
        ...
        if (mDual)
            dualViewport (mWindow, mRoot -> getSceneManager ("primary"), mRoot -> getSceneManager ("secondary") );

        else
            singleViewport (mWindow, mRoot -> getSceneManager ("primary") );
    }
    ...
    else if (arg.key == OIS::KC_C)
    {
        ...
        {
            if (mDual)
                dualViewport (mWindow, mRoot -> getSceneManager ("secondary"), mRoot -> getSceneManager ("primary") );

            else
                singleViewport (mWindow, mRoot -> getSceneManager ("secondary") );
        }
