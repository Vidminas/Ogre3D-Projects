...
Listeners::Listeners (Ogre::Root*& root, Ogre::RenderWindow*& window, Ogre::Camera*& camera, OgreBites::SdkCameraMan*& cameraMan, std::vector<Robot*>& robots) :
    mRoot           (root),
    mWindow         (window),
    mCamera         (camera),
...
bool Listeners::frameRenderingQueued (const Ogre::FrameEvent& evt)
{
    ...
    for (unsigned short i (0); i < mRobots.size(); i++)
        mRobots.at (i) -> animation (evt.timeSinceLastFrame);
