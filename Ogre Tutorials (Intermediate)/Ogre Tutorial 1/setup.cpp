...
void go()
{
    ...
    // Create the scene
    createScene (sceneMgr, robots);

    Listeners* listeners = new Listeners (root, window, camera, cameraMan, robots);
...
void createCamera (Ogre::SceneManager* sceneMgr, Ogre::Camera*& camera, OgreBites::SdkCameraMan*& cameraMan)
{
    ...
    camera -> setPosition (Ogre::Vector3 (1750, 5000, 1000) );
    camera -> lookAt (1750, 0, 500);
